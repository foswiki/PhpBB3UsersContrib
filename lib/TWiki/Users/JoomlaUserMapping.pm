...
sub login2canonical {
    ...
    $canonical_id = 'JoomlaUserMapping_'.$canonical_id;
...
#	return $TWiki::cfg{DefaultUserWikiName} if ($user == -1);	
	return $TWiki::cfg{DefaultUserWikiName} if ($user =~ /^JoomlaUserMapping_-1$/);	
	
	my $user_number = $user;
    $user_number =~ s/^JoomlaUserMapping_//;
	my $name;
...
#TODO: this needs to be implemented in terms of a DB iterator that only selects partial results
    ...
    for my $row (@$userDataset) {
        push @list, 'JoomlaUserMapping_'.$$row{id};		
	}
     ...
     if( exists $$groupIdDataSet[0] ) {
		...
		#TODO: re-write with join & map
			for my $row (@$groupDataset) {
				...
				my $user_id = 'JoomlaUserMapping_'.$$userDataset[0]{value};	# user_id
...
print STDERR "fetchPass($user)";    
    ...
    if( $user ) {
            my $dataset = $this->dbSelect('select * from jos_users fecthPass where fecthPass.username = ?', $user );
...
#	print STDERR "ASSERT_IS_CANONICAL_USER_ID($user_id)";
    ASSERT( ($user_id =~/^JoomlaUserMapping_-1$/) || ($user_id =~/^JoomlaUserMapping_\d+$/) );	#un-signed INT
